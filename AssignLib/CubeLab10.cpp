...
void CubeLab10::KeyCallback(GLFWwindow* window, int key, int scanCode, int action, int mods)
{
	if (key == GLFW_KEY_A)
		cameraXAngle += 5;
	if (key == GLFW_KEY_S)
...
void CubeLab10::Draw( const glm::mat4 &view, const glm::mat4 &proj )
{
	...
	if (useColor)
	{
		pShader->SetUniformVec4("fixedColor", glm::vec4(0.0, 1.0, 0.0, 1.0));			//Used to set the shader colour from interpolated to the Interpolate Colour boolean value
	...
	//if (colour) {										//Toggle the rendering of colour on each face of the cube
	//	pShader->SetUniformInt("useFixedColor", 1);
	//	pGeometry->DrawPrimitives();
